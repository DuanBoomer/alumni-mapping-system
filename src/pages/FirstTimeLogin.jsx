...
export default function FirstTimeLogin() {
	...
	function onSubmit(data) {
		...
		if (userType === 'alumni') {
			// console.log('a');
			axios
				.put(`${API_BASE}/update/alumni/${data.email}`, {
					...data,
					image: userImage,
			...
			// console.log('s');
			axios
				.put(`${API_BASE}/update/student/${data.email}`, {
					...data,
					image: userImage,
				})
				.then((response) => {
		...
		axios
			.put(`${API_BASE}/set/password/${data.email}`, {
		...
		<div style={{ padding: '1em 1em 3em 1em' }}>
			...
			<form onSubmit={handleSubmit(onSubmit)}>
				...
				<FormInputField
					...
					regexErrorMessage={'email must be of the valid format'}
					placeholder={'email'}
					register={register}
					errors={errors}
				/>
				{proceed === 0 ? (
					...
					userType === 'alumni' ? (
						<div>
							<FormInputField
								...
								label={'name'}
								regex={/[A-Za-z]+/i}
								regexErrorMessage={'name must be all text'}
							...
							<FormInputField
								...
								label={'password'}
								regex={
									/^\S*(?=\S{6,})(?=\S*\d)(?=\S*[A-Z])(?=\S*[a-z])(?=\S*[!@#$%^&*? ])\S*$/i
								}
								regexErrorMessage={
									'Minimum 6 characters, 1 upper case English letter, 1 lower case English letter, 1 special character'
								}
							...
							<FormInputField
								...
								regex={/[A-Za-z0-9]+/i}
								regexErrorMessage={
									'dont write code in the description, you hacker'
								}
								placeholder={'a really cool description'}
								register={register}
								errors={errors}
							/>
							<FormInputField
					...
					<div>
						<p
							...
							either email does not exist or you already have logged in
						</p>
						<Button
							text={'verify'}
							onClick={handleSubmit(verifyEmail)}
